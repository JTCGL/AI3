...
Editor View or Scene Camera; it preserves the persistent orbit-style Editor View state and optionally
...
Editor View is the non-authored viewport-owned view source. Its current implementation uses an orbit-style
pose and pivot; orbit is an operation on that view rather than the source's identity. A Perspective Camera
remains an ordinary scene object with its normal
identity, hierarchy, local transform, and semantic projection parameters. Camera selection belongs to a
viewport; AI3 has no global active scene camera. Selecting a scene camera retains the independent Editor View,
...
Navigation intent enters through `ViewportView` and mutates only Editor View state. Retained left-drag orbit
requires Navigation mode. An MMB gesture acquired over the viewport freezes either pan or orbit according to
whether Alt was held, continues with that operation until release, and does not change the retained interaction
mode. Wheel zoom operates in either retained mode. Navigation against a Scene Camera is inert because its
transform is authoritative object data, not retained viewport navigation state. Selection uses
display-independent CPU picking: normalized viewport coordinates and
...
workspace changes cannot reinterpret the drag. Scene Camera views use their resolved basis exactly as Editor
View does. The existing Selection/Navigation enum is not a Move/Rotate/Scale tool enum.
...
Viewport selection, interaction-mode state, Editor View navigation, sphere picking, scene-camera hierarchy
...
and the viewport to the default Editor View through their owning application model instances.
