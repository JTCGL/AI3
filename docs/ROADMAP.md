...
translated. The next navigation foundation distinguishes the non-authored Editor View from Scene Cameras:
middle-button drag temporarily pans, Alt+middle-button drag temporarily orbits, and wheel scrolling zooms the
Editor View without changing the retained interaction mode. Scene Camera data remains unaffected. The retained
mode and toolbar model may be refined only through later explicit editing-tool and viewport-control design. No
generalized tool or toolbar framework is established.
...
ImGui. Editor View navigation dispatch and sphere picking are also display-independent, while Scene Camera navigation
