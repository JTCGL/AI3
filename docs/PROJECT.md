...
- `scene`: display-independent units, transform and camera math, procedural sphere geometry, render-target
  sizing, orbit-style Editor View construction, viewport-view selection/resolution and interaction mode,
  sphere picking,
...
dialogs for Open and Save As. After successful Open, the UI resets the viewport to the default Editor View and
clears
...
is Editor View or Scene Camera. The Editor View's retained orbit-style pose and pivot remain independent of
scene hierarchy. Scene-camera selection belongs
...
the Editor View: retained Navigation-mode left drag orbits, MMB temporarily pans, Alt+MMB temporarily orbits,
and wheel input zooms in either retained mode. Transient gestures preserve their acquisition-time operation.
...
camera falls back deterministically to the unchanged Editor View; Reset Scene resets the viewport to its
default Editor View. `ViewportRenderer` consumes only resolved view/projection values and does not know which mode
...
content region after framebuffer scaling. Dear ImGui presents that texture in the visible editor window.
Display-independent view semantics are already separated from UI ownership, but construction and lifetime of
`ViewportRenderer` and its GLES resources remain inside `EditorUi`.

Navigation intent is dispatched through `ViewportView`. Retained left-drag orbit requires Navigation mode,
while MMB pan, Alt+MMB orbit, and wheel zoom temporarily operate in either retained mode. All operate only on
the Editor View; a Scene Camera remains authoritative scene data and is never changed by viewport navigation.
Selection mode constructs a bounded world ray from normalized viewport
